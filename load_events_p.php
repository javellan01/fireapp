...
/* 	function data_sql($data) {
	...
	//Calendario.php --- Carrega todas as atividades desse cliente no calendario
		...
		if($_SESSION["catuser"] == 0){
			try{
		$stmt = $conn->query("SELECT a.*,p.tx_codigo,cat.tx_nome FROM atividade a INNER JOIN pedido p ON a.id_pedido = p.id_pedido INNER JOIN categoria cat ON a.id_categoria = cat.id_categoria WHERE a.id_pedido = ".$pid."");
				}
